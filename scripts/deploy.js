...
const PAYMASTER_STAKE = hardhat_1.ethers.utils.parseEther('1');
async function main() {
    const EntryPoint = await hardhat_1.ethers.getContractFactory("EntryPoint");
    ...
    const WalletLogic = await hardhat_1.ethers.getContractFactory("SmartWallet");
    ...
    const Create2Factory = await hardhat_1.ethers.getContractFactory("Create2Factory");
    ...
    const WETHPaymaster = await hardhat_1.ethers.getContractFactory("WETHTokenPaymaster");
    const wethPaymaster = await WETHPaymaster.deploy(entrypoint.address, "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266", weth.address);
